...
def load_investor_details(investor):
    ...
    with col4:
        city_series = df[df['investors'].str.contains('IDG Ventures')].groupby('city')['amount'].sum()
        st.subheader('Cities')
...
def load_overall_analysis():
    ...
    with col3:
        st.metric('Avg', str(round(avg_funding, 2)) + ' Cr')
    with col4:
        st.metric('Funded started', str(num_startups) + ' Cr')

    st.header('MonthOnMonth Graph')
